...
/* Двадцать страниц, которые человек видит сразу.

   Порядок не случайный: на компьютере сетка ставит их по шесть в ряд,
   и последние два всегда висят неполным рядом. Поэтому на этих двух
   местах стоят роза и вишня, а клубника и пончик подняты выше: на
   широком экране два последних рисунка не показываются вовсе, ряды
   получаются полными, а лучшие страницы остаются на виду.

   На телефоне и на планшете рядов по два, четыре или пять, двадцать
   делится на каждое из этих чисел без остатка, и там видны все. */
export const featured: number[] = [
  1, 23, 22, 24, 16, 29, 34, 9,
  41, 48, 47, 46,
  26, 67, 69, 72,
  111, 106,
  /* Последние два. На широком экране не показываются. */
  89, 104,
];
...
/* Уровень сложности каждого рисунка книги: 1 самый простой, 3 самый
   подробный. Нужен инструменту подбора: в ответе он показывает
   примеры страниц, подходящих этому этапу, и примеры должны
   отличаться, иначе показывать их незачем.

   Уровень посчитан, а не назначен на глаз. Два признака, оба видны
   на самом рисунке:

   первый, сколько внутри отдельных участков, которые можно закрасить
   разным цветом. Скат это один участок, бадминтон двадцать три;

   второй, толщина контура. У самых простых она доходит до четырех
   с половиной миллиметров на листе формата US Letter, у самых
   подробных падает до одного и семи.

   Границы: до семи участков первый уровень, до тринадцати второй,
   дальше третий. Числа проверяемые: любой может распечатать
   бесплатный лист и приложить линейку.

   Здесь лежат не все 111, а по восемь показательных на уровень,
   разнесенных по темам книги. Инструменту больше и не нужно, а
   держать вручную выверенный список честнее, чем считать сложность
   на лету и надеяться, что не выпадет что-нибудь неудачное.

   Рисунки 65, 89 и 95 когда-то шли со вшитым в картинку словом,
   причем у шляпы волшебника и у ландыша оно было еще и обрезано:
   "Wizard's" без "hat", "Lily of the" без "valley". На русской и
   испанской странице это выглядело как обрезанное английское слово
   внутри рисунка. Файлы перерисованы: слово убрано, рисунок заново
   отцентрован и приведен к тому же заполнению, что у остальных
   ста восьми. Теперь они ничем не отличаются и участвуют наравне. */

export const examplesByLevel: Record<1 | 2 | 3, number[]> = {
  1: [55, 88, 14, 38, 49, 102, 61, 107],
  2: [4, 15, 52, 60, 70, 71, 79, 85],
  3: [77, 39, 92, 51, 46, 63, 21, 94],
};

/** Примеры страниц для этапа. Уровень тем выше, чем увереннее рука. */
const LEVEL_FOR_STAGE: Record<string, 1 | 2 | 3> = {
  scribble: 1,
  control: 1,
  aim: 2,
  shape: 3,
...
export const examplesForStage = (stageId: string, n = 3) => {
  const level = LEVEL_FOR_STAGE[stageId] ?? 1;
  return examplesByLevel[level].slice(0, n).map(drawingByNumber).filter(Boolean) as Drawing[];
};
