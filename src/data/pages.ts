...
export type SectionCopy = {
  title: string;
  /* Строка, которую видно в выдаче поиска. Заполняется только там,
     где заголовок страницы слишком длинный: Google обрезает строку
     по ширине экрана, и у длинных заголовков в выдаче оставался
     хвост без главного слова. Если поле пустое, берется title,
     то есть обычный заголовок страницы. Заголовок, который читает
     человек на странице, это поле не меняет никогда. */
  metaTitle?: string;
  lead: string;
...
export const agesCopy: Record<ContentLang, SectionCopy> = {
  ...
  ru: {
    title: "Раскраски по возрасту: что подходит ребенку в год, два, три и четыре",
      metaTitle: "Раскраски по возрасту: от года до четырех лет",
    lead:
...
export const faqCopy: Record<UiLang, SectionCopy> = {
  en: {
    ...
    body: [
      "These are the questions parents ask most often. Pick a topic and open your question.",
    ],
  },
  es: {
    ...
    body: [
      "Aquí están las preguntas que más hacen las familias. Elija un tema y abra su pregunta.",
    ],
  },
  ru: {
    ...
    body: [
      "Здесь вопросы, которые родители задают чаще всего. Выберите тему и откройте свой вопрос.",
    ],
  },
};
