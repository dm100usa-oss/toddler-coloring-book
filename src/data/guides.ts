...
/* ------------------------------------------------------------------ */
/*  Связь статей с этапами                                             */
/* ------------------------------------------------------------------ */

/* Статьи и страницы этапов до сих пор были двумя отдельными островками:
   из статьи можно было уйти на этап, а из этапа на статью нет. Родитель
   на странице этапа читает, что рука ребенка пока движется от плеча,
   и следующий его вопрос всегда один и тот же: а чем тогда рисовать.
   Ответ на него лежит в статье, до которой без этой связи надо идти
   через меню.

   Для машины это важнее, чем для человека. Страница, на которую ведут
   ссылки только из своего же раздела, выглядит второстепенной. Здесь
   каждая статья получает ссылку еще и со страницы этапа, а этап со
   страницы статьи, и обе перестают быть тупиками.

   Две статьи, про виды раскрасок и про надписи на упаковке, к одному
   этапу не привязаны: они нужны на любом. Поэтому они добираются
   к каждому этапу вторым слоем, чтобы под заголовком не оставалось
   одинокой ссылки. */
export const guidesForStage = (stageId: StageId, n = 3): Guide[] => {
  const own = guides.filter((g) => g.stage === stageId);
  const general = guides.filter((g) => !g.stage);
  return [...own, ...general].slice(0, n);
};
