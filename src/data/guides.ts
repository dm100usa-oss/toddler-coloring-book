...
export const guides: Guide[] = [
  /* ---------------------------------------------------------------- */
  {
    ...
    lead: {
      ...
      ru:
        "Большинство детей делает первые осознанные черты на бумаге между двенадцатью и " +
        "восемнадцатью месяцами, держа мелок в кулаке. Это и есть настоящее начало. " +
        "Оставаться внутри контура это отдельное умение, оно приходит ближе к трем годам, и " +
        "ждать его, прежде чем дать ребенку мелок, значит ждать примерно на полтора года дольше.",
    },
    body: {
      en: [
      ...
      ru: [
        "За этим вопросом обычно стоит другой: не когда ребенок сможет удержать мелок, а когда " +
        "мелок перестанет быть переводом бумаги. Честный ответ в том, что переводом бумаги это " +
        "не бывает никогда, потому что в год ребенок учится совсем не рисунку. Он учится тому, " +
        "что его собственное движение меняет мир за пределами тела. В этом весь урок, и " +
        "исчерканный лист доказывает, что урок дошел.",

        "Примерно с двенадцати до пятнадцати месяцев ребенок способен оставить след и нарочно " +
        "повторяет движение, чтобы увидеть след снова. Хват кулаком, движение идет от плеча и " +
        "локтя, черты длинные и размашистые. Ничто из этого никуда не целится.",

        "Примерно от восемнадцати месяцев до двух лет каракули становятся осознанными. Ребенок " +
        "начинает черкать сам, без показа, может повторить линию, которую только что нарисовал " +
        "кто-то другой, и нарочно делает круглые петли и черты сверху вниз. Теперь большая часть " +
        "следов попадает на рисунок, а не на стол вокруг.",

        "Между двумя и тремя годами ребенок начинает целиться. Он повторяет вертикальную линию, " +
        "горизонтальную линию и круг и попадает в рисунок большей частью цвета, свободно выходя " +
        "за контур. Этот этап родители чаще всего принимают за неудачу. Выход за контур здесь " +
        "не ошибка, а то, как выглядит попадание, пока рука не догнала глаз.",

        "Около трех лет, а увереннее ближе к четырем, оставаться внутри контура становится " +
        "возможно, и ребенок начинает замечать, когда цвет вышел наружу. Крестик обычно " +
        "повторяется годам к четырем. В этой же точке книга, сделанная для первого этапа, " +
        "начинает казаться слишком легкой.",

        "Практическое правило для первого мелка простое. Дайте толстый восковой мелок, а не " +
        "простой карандаш и не фломастер. Посадите ребенка за стол с одним листом, а не с целой " +
        "книгой. Ждите, что первые занятия продлятся две-три минуты и закончатся тогда, когда " +
        "ребенок встанет, а не тогда, когда страница закончена. Ничего из этого чинить не надо. " +
        "Именно так начало и выглядит.",
      ],
    },
    faq: {
      en: [
      ...
      ru: [
        {
          ...
          a:
            "Да, если страница подходящая. В год ребенок держит мелок в кулаке и делает " +
            "размашистые черты, никуда не целясь, поэтому странице нужен очень толстый контур и " +
            "одна крупная форма во весь лист. На такой странице любая черта попадает туда, где " +
            "она заметна, и ребенок получает результат. На странице с мелкими подробными " +
            "рисунками ничего заметного не происходит, и интерес пропадает меньше чем за минуту.",
        },
        {
          q: "Безопасно ли давать мелки годовалому?",
          a:
            "Берите мелки с пометкой о нетоксичности и достаточно крупные, чтобы их нельзя было " +
            "проглотить, и оставайтесь за столом, пока ребенок ими занят. В этом возрасте дети " +
            "тянут вещи в рот, и толстый мелок выбирают не только ради хвата.",
        },
        {
          q: "Сколько должно длиться первое занятие?",
          a:
            "Две-три минуты в самом начале это нормально и не говорит о плохом внимании. " +
            "Заканчивайте, когда ребенок встал, а не когда страница закончена. Занятия " +
            "удлиняются сами за несколько месяцев, по мере того как ребенок получает от них больше.",
        },
      ],
    },
  },
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    lead: {
      ...
      ru:
        "Сначала толстые восковые мелки, как минимум первый год рисования. Они не требуют силы " +
        "в руке, не рвут лист, не проходят бумагу насквозь и оставляют широкий след, который " +
        "ребенок действительно видит. Фломастеры приходят позже, цветные карандаши еще позже, и " +
        "оба по конкретным причинам, а не как повышение уровня.",
    },
    body: {
      en: [
      ...
      ru: [
        "Выбор значит больше, чем кажется, потому что в этом возрасте именно инструмент решает, " +
        "увидит ли ребенок результат. Ребенок, который давит изо всех сил и получает бледную " +
        "линию, делает вывод, что ничего не происходит, и бросает. Все остальное вытекает из " +
        "этого одного факта.",

        "Толстые восковые мелки, их же называют восковыми карандашами, годятся первыми по " +
        "четырем отдельным причинам. Они оставляют след почти без нажима, поэтому рука, в " +
        "которой еще нет силы, все равно дает заметную линию. Они достаточно толстые, чтобы их " +
        "держали кулаком, а именно так ребенок держит вещи, пока за дело не взялись пальцы. Они " +
        "не рвут бумагу, чего об остром карандаше в тяжелом кулаке не скажешь. И они не проходят " +
        "лист насквозь, поэтому закрашенная страница остается просто закрашенной страницей.",

        "Фломастеры дают самый яркий результат, и дети любят их именно за это. Плата за яркость " +
        "в том, что они проходят обычную бумагу насквозь, и на двусторонней печати рисунок с " +
        "обратной стороны пропадает, а еще они куда охотнее мелков остаются на столе, одежде и " +
        "коже. Смываемые фломастеры снимают большую часть этой проблемы. Запасной лист под " +
        "страницей снимает остальное.",

        "Цветные карандаши требуют хвата пальцами и ровного нажима, а ни того ни другого у " +
        "большинства детей до трех лет еще нет. Если дать их раньше, карандаш либо не оставляет " +
        "заметного следа, либо ломается. Свое время у них приходит, когда ребенок уже остается " +
        "внутри контура и хочет более тонкого управления, и это настоящее желание, торопить " +
        "которое незачем.",

        "Две мелочи, о которых стоит знать. Толстые трехгранные мелки или мелки в форме яйца " +
        "сами подсказывают пальцам место, где им в итоге придется стоять, и это мягкая " +
        "подсказка, а не тренажер. И ребенку-левше не нужны другие инструменты, но ему заметно " +
        "удобнее, когда рисунок стоит в середине листа, а не сдвинут к краю: левая рука во время " +
        "работы накрывает то, что лежит слева от нее.",
      ],
    },
    faq: {
      en: [
      ...
      ru: [
        {
          ...
          a:
            "Смываемые фломастеры с пометкой о нетоксичности широко используют в этом возрасте, " +
            "и разницу на коже, одежде и мебели делает именно смываемый состав. Бумагу они все " +
            "равно проходят насквозь, поэтому подкладывайте запасной лист. Колпачки держите вне " +
            "досягаемости: они достаточно малы, чтобы ребенок мог ими подавиться.",
        },
        {
          q: "Ребенок держит мелок в кулаке. Надо ли поправлять?",
          a:
            "Нет. Хват кулаком нормален и ожидаем примерно до двух с половиной лет, а поправлять " +
            "его силой обычно значит закончить занятие, а не исправить хват. Хват пальцами " +
            "приходит сам по мере того, как развивается рука. Толстые трехгранные мелки мягко " +
            "подсказывают его без единого слова.",
        },
        {
          q: "А краски в этом возрасте?",
          a:
            "Краски работают, и детям они нравятся, но это другое занятие, а не ступень выше " +
            "мелков. Они учат цвету и покрытию, а не попаданию и управлению, которые дает " +
            "раскраска. Большинство семей делает и то и другое и достает краски тогда, когда " +
            "есть время на уборку.",
        },
      ],
    },
  },
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    lead: {
      ...
      ru:
        "Почти вся грязь берется из трех вещей: неподходящий инструмент, страница, в которую " +
        "ребенку нечем целиться, и занятие, которое тянется дольше, чем ребенку было интересно. " +
        "Если поправить эти три вещи, убирать почти нечего, и это дает больше, чем любая " +
        "клеенка или фартук.",
    },
    body: {
      en: [
      ...
      ru: [
        "Полезно разделить две разные заботы, которые называют одним словом. Одна это следы " +
        "там, где их быть не должно: стол, стена, сам ребенок. Другая это то, что сама страница " +
        "выглядит хаотично. Вторая грязью не является вовсе, именно так и выглядит рисование в " +
        "этом возрасте, и относиться к нему как к беде это самый быстрый способ отбить у " +
        "ребенка удовольствие.",

        "Со следами не там, где надо, основную работу делает инструмент. Восковые мелки в " +
        "основном остаются там, куда их положили, и легко стираются с твердых поверхностей. " +
        "Смываемые фломастеры сделаны ровно для этой ситуации и сходят с кожи и с большинства " +
        "тканей. Обычному перманентному маркеру рядом с двухлетним ребенком делать нечего, как " +
        "и открытой банке краски на ковре.",

        "Для самой страницы решение это мишень. Ребенок черкает мимо листа в основном тогда, " +
        "когда на листе нет ничего, ради чего стоило бы на нем оставаться. Крупный рисунок с " +
        "толстым контуром дает руке цель, а цель удерживает мелок на бумаге куда надежнее, чем " +
        "просьба быть аккуратнее.",

        "Длительность занятия самая тихая из трех причин и самая надежная. Ребенок, у которого " +
        "интерес закончился, не откладывает мелок вежливо, он начинает проверять, что еще этим " +
        "мелком можно сделать. Если заканчивать занятие при первом таком признаке, а не по " +
        "концу страницы, почти все происшествия, которые родители потом вспоминают, просто не " +
        "случаются.",

        "Два практических приема стоят своих усилий. Подложите под страницу запасной лист " +
        "бумаги: он ловит и то, что проходит насквозь от фломастера, и то, что уезжает за края. " +
        "И давайте по одному листу, а не раскрытую книгу: это убирает соблазн перевернуть " +
        "страницу на середине рисунка и вдвое уменьшает площадь, на которой может что-нибудь " +
        "случиться.",

        "Одно замечание перед покупкой. Наборы, которые продают как раскраску без грязи, это " +
        "обычно другой товар: особые фломастеры, которые проявляются только на своей " +
        "пропитанной бумаге. Они работают как обещано и действительно чистые, но ребенок " +
        "раскрашивает фломастером, который больше нигде ничего не оставляет, и это другое " +
        "занятие, чем рисование на бумаге. Место есть у обоих. Друг друга они не заменяют.",
      ],
    },
    faq: {
      en: [
      ...
      ru: [
        {
          ...
          a:
            "Теплая вода с каплей средства для посуды на мягкой тряпке снимает большую часть " +
            "воска с крашеной стены, тереть надо мягко, чтобы не снять краску. Меламиновая губка " +
            "убирает остаток, но может сделать матовую поверхность более тусклой, поэтому " +
            "пробуйте сначала на незаметном месте. Посмотрите упаковку мелков: многие " +
            "производители пишут, чем оттирать именно их продукт.",
        },
        {
          q: "Раскраска без грязи это то же самое, что обычная раскраска?",
          a:
            "Нет. В наборах без грязи используют особые фломастеры, которые проявляются только " +
            "на своей пропитанной бумаге, поэтому ни стол, ни ребенок не пачкаются. Они " +
            "действительно чистые и удобны в дороге, но ребенок при этом не рисует на обычной " +
            "бумаге и не может взять тот же фломастер куда-то еще. Они работают рядом с бумажной " +
            "раскраской, а не вместо нее.",
        },
        {
          q: "Нужен ли ребенку фартук?",
          a:
            "С восковыми мелками почти никогда. С фломастерами или красками старая рубашка " +
            "удобнее настоящего фартука и работает не хуже. Больше всего экономит выбор " +
            "смываемых инструментов с самого начала.",
        },
      ],
    },
  },
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    lead: {
      ...
      es:
        ...
        "dibujo siguiente.",
      ru:
        "Обычная книжная бумага фломастер не держит, и почти ни одна раскраска в этой цене не " +
        "печатается на бумаге, которая держала бы. Запасной лист, подложенный под страницу, " +
        "решает это полностью и в одно движение. Книга с печатью только с одной стороны решает " +
        "это навсегда: тогда прошедший насквозь фломастер портит пустой оборот, а не следующий " +
        "рисунок.",
    },
    body: {
      en: [
      ...
      ru: [
        "Это самая частая жалоба в отзывах на детские раскраски, и стоит понять, почему так " +
        "происходит, а не считать это браком. Бумага, достаточно плотная, чтобы остановить " +
        "фломастер, стоит в несколько раз дороже за лист и делает книгу тяжелее и намного " +
        "дороже. Книга на такой бумаге стоила бы втрое или вчетверо больше, и для книги, " +
        "которую ребенок исчеркает за месяц, это обмен, на который большинство родителей не " +
        "пошло бы, если бы им его прямо предложили.",

        "Движение, которое все решает, занимает секунду. Положите под страницу, на которой " +
        "работает ребенок, чистый лист бумаги или кусок картона. Все, что пройдет насквозь, " +
        "останется на нем. Это работает с любой книгой, не стоит ничего, и заводить это стоит " +
        "как привычку, а не после первого испорченного рисунка.",

        "Решение на уровне выбора книги это печать только с одной стороны. Тогда оборот каждого " +
        "рисунка пуст изначально, и прошедший насквозь фломастер оставляет след на пустом " +
        "листе. Это стоит проверить до покупки: в просмотре первых страниц видно сразу, а на " +
        "обложке об этом пишут редко. Книга, где рисунок стоит с обеих сторон каждого листа, " +
        "теряет две картинки каждый раз, когда в руки берут фломастер.",

        "Другой путь это менять инструмент, а не бумагу. Восковые мелки не проходят насквозь " +
        "вовсе. Цветные карандаши тоже. Насквозь проходят только фломастеры и краски, и именно " +
        "за фломастером ребенок тянется первым, потому что цвет ярче. Если в конкретный день " +
        "чистый оборот важнее яркости, это решение на две секунды у коробки с мелками.",

        "Чего делать не надо: проглаживать лист утюгом, сушить его феном и пытаться снять " +
        "чернила фломастера растворителем. На книжной бумаге не работает ни то ни другое, и " +
        "обычно небольшая неприятность превращается в порванную страницу.",
      ],
    },
    faq: {
      en: [
      ...
      ru: [
        {
          ...
          a:
            "Это зависит от плотности бумаги, и почти ни одна детская раскраска в обычной цене " +
            "не печатается на бумаге, достаточно плотной, чтобы остановить фломастер. Хорошую " +
            "книгу от плохой здесь отличает не бумага, а печать: при односторонней печати " +
            "прошедший насквозь фломастер оставляет след на пустом обороте, а не на следующем " +
            "рисунке.",
        },
        {
          q: "Проходят ли насквозь восковые мелки?",
          a:
            "Нет. Восковой мелок остается на поверхности бумаги, а не впитывается в нее, поэтому " +
            "на другую сторону он не выходит ни при каком нажиме, на который способен маленький " +
            "ребенок. То же верно для цветных карандашей.",
        },
        {
          q: "Можно ли все-таки пользоваться фломастерами на тонкой бумаге?",
          a:
            "Да, если подложить запасной лист. Он принимает на себя все, что проходит насквозь, " +
            "и ребенок разницы не замечает. Это самая простая привычка из возможных, и проблему " +
            "она снимает целиком.",
        },
      ],
    },
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    title: {
      ...
      ru: "Сколько маленький ребенок на самом деле сидит за раскраской?",
    },
    lead: {
      en:
      ...
      ru:
        "Две-три минуты в самом начале, около пяти минут у большинства двухлетних и десять " +
        "минут и дольше, когда ребенок уже остается внутри контура и выбирает цвета нарочно. " +
        "Занятие, которое закончилось тем, что ребенок встал, это нормальное занятие, а не " +
        "сорванное.",
    },
    body: {
      en: [
      ...
      ru: [
        "Родители обычно задают этот вопрос потому, что занятие кончилось намного быстрее " +
        "ожидаемого, и они пытаются понять, все ли в порядке. Почти всегда все в порядке. " +
        "Поправить надо ожидание, а не ребенка.",

        "Внимание в этом возрасте не постоянная величина. Один и тот же ребенок проведет " +
        "полторы минуты над одной страницей и восемь минут над следующей, и разница обычно в " +
        "странице, а не в дне. Рисунок, который ребенок узнает и может назвать, держит его " +
        "дольше незнакомого по простой причине: назвать это половина того, что делает рисунок " +
        "интересным.",

        "Три вещи надежно удлиняют занятие, и ни одна из них не состоит в просьбе " +
        "сосредоточиться. Знакомый предмет: собака удержит двухлетнего лучше, чем затейливое " +
        "сказочное существо. Рисунок достаточно крупный, чтобы продвижение было видно уже " +
        "после нескольких движений. И взрослый, который сидит за тем же столом и раскрашивает " +
        "свой лист, а это работает лучше любых подбадриваний из другого конца комнаты.",

        "Две вещи надежно укорачивают его. Просьба оставаться внутри контура раньше, чем " +
        "ребенок на это способен, превращает приятное занятие в проверку, которую он раз за " +
        "разом не проходит. И продолжение после того, как интерес закончился, а это ровно тот " +
        "момент, когда мелок начинает пробовать стол.",

        "Занятия удлиняются сами за несколько месяцев. Тренировать тут нечего, и никаких " +
        "упражнений делать не надо. Меняются рука, хват и то, сколько ребенок получает от " +
        "самого занятия, и все три вещи двигаются вперед без всяких усилий со стороны " +
        "взрослого.",
      ],
    },
    faq: {
      ...
      ru: [
        {
          ...
          a:
            "Да. Короткие занятия обычны в два года, и за несколько месяцев они удлиняются сами, " +
            "без всякого вмешательства. Попробуйте рисунок, который ребенок может назвать " +
            "вслух, и сядьте раскрашивать свой лист рядом: и то и другое надежно добавляет " +
            "времени, не требуя от ребенка сосредоточиться.",
        },
        {
          q: "Надо ли заставлять ребенка дораскрасить страницу?",
          a:
            "Нет. Законченная страница это взрослое представление о том, зачем нужна раскраска. " +
            "В этом возрасте ценность в сделанных движениях, а не в полном покрытии, и " +
            "требование закончить это самый частый путь к тому, что раскрашивать ребенок " +
            "разлюбит.",
        },
        {
          q: "Улучшает ли раскрашивание внимание?",
          a:
            "Раскрашивание одно из многих занятий, в которых дети упражняются оставаться при " +
            "одном деле, наравне со строительством, разбором по кучкам и разглядыванием книг. " +
            "Время, потраченное так, не пропадает, но внимание в этом возрасте определяется в " +
            "основном обычным ходом развития, а не каким-то отдельным занятием.",
        },
      ],
    },
  },
